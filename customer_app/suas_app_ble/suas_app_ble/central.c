...
uint8_t ble_central_discovery_function(struct bt_conn *conn,
    ...
    if (!bt_uuid_cmp(discover_params.uuid, BT_UUID_TEST)) { // BT_UUID_TEST received
        ...
        discover_params.type = BT_GATT_DISCOVER_CHARACTERISTIC; // discover characteristic
        ...
        subscribe_params.value_handle = bt_gatt_attr_value_handle(attr); // returns handle of current attribute
        ...
        subscribe_params.notify = ble_central_notify_function; // Register callback function
...
void ble_central_connected(struct bt_conn *conn, uint8_t conn_err) {
    ...
    if (conn == default_conn) {
        ...
        discover_params.end_handle = 0xFFFF; // Included service endle handle (check for UUIDs between both start and end values)
        discover_params.type = BT_GATT_DISCOVER_PRIMARY; // Service type to discover
...
void ble_central_device_found(const bt_addr_le_t *addr, int8_t rssi, uint8_t type,
