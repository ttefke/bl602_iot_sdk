...
void bfl_main(void) {
  ...
  xTaskCreate(task_two,              /* function implementing the task */
              (char *)"second task", /* human readable task name */
              STACK_SIZE,            /* stack depth */
              (void *)42, /* parameter handed over -> number, must be casted to
                             void pointer */
              10,         /* task priority */
              &second_task_handle /* task handle */
  );

  xTaskCreate(task_three, (char *)"third task", STACK_SIZE,
              (void *)second_task_handle, 10, NULL);
