...
extern "C" void bfl_main(void) {
  ...
  constexpr uint16_t HTTPD_STACK_SIZE = 512;
  constinit static StackType_t httpd_stack[HTTPD_STACK_SIZE]{};
  constinit static StaticTask_t httpd_task{};
  ...
  xTaskCreateStatic(task_httpd, etl::string_view("httpd").data(),
                    HTTPD_STACK_SIZE, nullptr, 10, httpd_stack, &httpd_task);
