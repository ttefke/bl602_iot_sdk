...
void my_mqtt_publish() {
  ...
  if (!mqtt_connected) {
    ...
    err = mqtt_publish(&mqtt_client, subscribed_topic, payload, strlen(payload),
                       QUALITY_OF_SERVICE, retain, my_mqtt_publish_cb, 0);
...
void my_mqtt_connected_cb([[gnu::unused]] mqtt_client_t *client, void *arg,
  ...
  if (status == MQTT_CONNECT_ACCEPTED) {
    ...
    err_t err = mqtt_subscribe(&mqtt_client, subscribed_topic,
...
void my_mqtt_connect() {
  ...
  client_info.tls_config = altcp_tls_create_config_client_2wayauth(
      ca_certificate, ca_certificate_len, private_key, private_key_len, NULL, 0,
      certificate, certificate_len);

  // Initiate connection (TLS)
  err = mqtt_client_connect(&mqtt_client, &mqtt_broker_ip, MQTT_TLS_PORT,
                            my_mqtt_connected_cb, 0, &client_info);
#else
  // Initiate connection (plain text)
  err = mqtt_client_connect(&mqtt_client, &mqtt_broker_ip, MQTT_PORT,
                            my_mqtt_connected_cb, 0, &client_info);
#endif
