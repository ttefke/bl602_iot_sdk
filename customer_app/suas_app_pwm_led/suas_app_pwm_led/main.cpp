...
extern "C" void bfl_main(void) {
  ...
  constinit static StackType_t led_stack_pwm[LED_STACK_SIZE]{};
  constinit static StaticTask_t led_task_pwm{};
  ...
  /* Create the task */
  xTaskCreateStatic(
      task_led_pwm, /* name of the function implementing the task -> defined in
                       led_pwm.c */
      etl::string_view("led pwm").data(), /* human readable name of the task */
      LED_STACK_SIZE,                     /* Stack size */
      nullptr,       /* parameters for the function -> not required here */
      15,            /* Task priority */
      led_stack_pwm, /* Stack to use for the task */
      &led_task_pwm /* Task handle (could be referenced in API calls later, e.g.
                       for changing its priority )*/
  );
