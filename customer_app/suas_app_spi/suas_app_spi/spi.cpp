...
constexpr uint8_t MAX_CARD_LEN = 10;  // RFID UIDs have a length of at most 10
...
static bool is_card_allowed(suas_rfid_uid_t card) {
  // Iterates over all cards
  for (/* auto type-infer does not work here*/ uint8_t i = 0;
       i < sizeof(allowed_cards) / MAX_CARD_LEN; i++) {
    // Iterate over the individual bytes of each card
    for (auto j = 0; j < card.size; j++) {
      if (allowed_cards[i][j] != card.uid_byte[j]) {
        break;  // go to outer loop to try for next card
      } else if (j == (card.size - 1)) {
        // We are at the last byte and this as well as all previous bytes
        // matched
        return true;
      }
    }
...
void spi_proc([[gnu::unused]] void *pvParameters) {
  ...
  // Initialize card reader regularly to avoid some SPI HAL hiccups
  suas_rfid_init();
  ...
  while (1) {
    ...
    if (suas_rfid_is_new_card_present() && suas_rfid_read_card()) {
      ...
      auto card_data = suas_rfid_get_uid();
      for (auto i = 0; i < card_data.size; i++) {
  ...
  // Delete task if it exits - should not happen
  vTaskDelete(nullptr);
}