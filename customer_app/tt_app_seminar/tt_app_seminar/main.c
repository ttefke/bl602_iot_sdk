...
void bfl_main(void)
{
  ...
  xTaskCreateStatic(task_grove_dls, (char*)"grove dls", GROVE_STACK_SIZE, NULL, 15, grove_stack, &grove_task_dls);

  printf("[SYSTEM] Starting WiFi task\r\n");
  extern void task_wifi(void *pvParameters);
  xTaskCreateStatic(task_wifi, (char*)"wifi", WIFI_STACK_SIZE, NULL, 17, wifi_stack, &wifi_task);

  printf("[SYSTEM] Starting TCP/IP stack\r\n");
  extern void task_tcp(void *pvParameters);
  xTaskCreateStatic(task_tcp, (char*)"tcp", TCP_STACK_SIZE, NULL, 18, tcp_stack, &tcp_task);

  printf("[SYSTEM] Starting http task\r\n");
  extern void task_http(void *pvParameters);
  xTaskCreateStatic(task_http, (char*)"http", HTTP_STACK_SIZE, NULL, 16, http_stack, &http_task);
